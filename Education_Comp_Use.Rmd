...
data_enum2 <- data %>%
                ...
                summarize(`Less than high school with dialup` = mean(`< HS w/Comp w/Dial`),
                          `Less than high school no Internet` = mean(`< HS w/Comp w/out IS`),
                          `High school with dialup` = mean(`HS some Col w/Comp w/Dial`),
                          `High school with no internet` = mean(`HS some Col w/Comp w/out IS`),
                          `College or more with dialup` = mean(`BA+  w/Comp w/Dial`),
                          `College or more with no internet` = mean(`BA+  w/Comp w/out IS`))
...
ggplot(data, aes(x = Region, y = Prop_LHS_No_Comp_Intrnt)) +
  geom_bar(stat = "identity", fill = "red", color = "black") +
  labs(title = "% of people lower than HS w/o Computers or Internet Access in each state in the US", x = "State", y = "% of people w/ < HS educ and w/o IS") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
#<<<<<<< HEAD
