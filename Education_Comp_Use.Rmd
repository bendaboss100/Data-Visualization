...
data_enum2 <- data %>%
                group_by(Region) %>%
                summarize(mean_lhs_dial = mean(`< HS w/Comp w/Dial`),
                          mean_lhs_noIS = mean(`< HS w/Comp w/out IS`),
                          mean_hs_dial = mean(`HS some Col w/Comp w/Dial`),
                          mean_hs_noIS = mean(`HS some Col w/Comp w/out IS`),
                          mean_col_dial = mean(`BA+  w/Comp w/Dial`),
                          mean_col_noIS = mean(`BA+  w/Comp w/out IS`))

data_enum2 <- data_enum2 %>% pivot_longer(cols = contains("mean", ignore.case = FALSE), names_to = "Tech_Access_Group", values_to = "Count")
```
...
ggplot(data_enum2, aes(x = Region, y = Count, fill = Tech_Access_Group)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Breakdown of number of people with different tech access levels in each state",
       x = "Region",
       y = "Count",
...
rm(list = ls())
library(sf) 
path="Data/cb_2018_us_state_500k.json"
stateMap=sf::read_sf(path, layer='cb_2018_us_state_500k')

base_map= ggplot(data = stateMap)
...
state_area2 <- data.frame(State = state.name, Area = state.area)


variables2 <- data[, c("State", "Region", "State Total", "Prop_no_Comp" )]
...
state_area2 <- rbind(state_area2, extra_rows)

variables2 <- merge(variables2, state_area2, by = "State", all.x = TRUE)

variables2 <- variables2 %>% 
  mutate(State_Population_Density = `State Total` / state_area)

variables2 <- variables2 %>% 
  mutate(percent_no_comp_pop_density = Prop_no_Comp / State_Population_Density)

variables2 <- variables2 %>% mutate(log_percent_no_comp_pop_density = log(percent_no_comp_pop_density))

```

```{r}

popdensity <- ggplot(variables2, aes(x = log(State_Population_Density), y = Prop_no_Comp)) +
  geom_histogram(stat = "identity", binwidth=100, color="blue", fill="skyblue") +  
  labs(x = "Log of Population Density", y = "Proportion of People without Computers") +
  ggtitle("USA: Proportion of people without computers by Population Density") + 
  theme_minimal() +
